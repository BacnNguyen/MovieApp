...
@AndroidEntryPoint
class ShowAiringTodayFragment : BaseFragment<FragmentCategoryMovieBinding>(R.layout.fragment_category_movie) {

    private var isList = true

    private val viewModel by viewModels<ShowAiringTodayViewModel>()
    ...
    private fun onMovieClick(item: TvShowResponse.TvShow, position: Int) {
        val action = ShowAiringTodayFragmentDirections.actionShowAiringTodayToTvShowDetail(item.id)
        findNavController().navigate(action)
    }

    companion object {
        const val TAG = "ShowAiringTodayFragment"

        fun newInstance() = ShowAiringTodayFragment()
    }
