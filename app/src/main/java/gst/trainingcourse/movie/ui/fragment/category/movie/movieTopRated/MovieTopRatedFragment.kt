...
@AndroidEntryPoint
class MovieTopRatedFragment : BaseFragment<FragmentCategoryMovieBinding>(R.layout.fragment_category_movie) {

    private var isList = true

    private val viewModel by viewModels<MovieTopRatedVIewModel>()
    ...
    private fun onMovieClick(item: MovieResponse.Movie, position: Int) {
        val action = MovieTopRatedFragmentDirections.actionMovieTopRatedToMovieDetail(item.id)
        findNavController().navigate(action)
    }

    companion object {
        const val TAG = "MovieTopRatedFragment"
