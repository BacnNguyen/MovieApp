...
@AndroidEntryPoint
class ShowOnTheAirFragment :
    BaseFragment<FragmentCategoryMovieBinding>(R.layout.fragment_category_movie) {

    private var isList = true

    private val viewModel by viewModels<ShowOnTheAirViewModel>()
    ...
    private fun onMovieClick(item: TvShowResponse.TvShow, position: Int) {
        val action = ShowOnTheAirFragmentDirections.actionShowOnTheAirToTvShowDetail(item.id)
        findNavController().navigate(action)
    }

    companion object {
        const val TAG = "ShowOnTheAirFragment"

        fun newInstance() = ShowOnTheAirFragment()
    }
